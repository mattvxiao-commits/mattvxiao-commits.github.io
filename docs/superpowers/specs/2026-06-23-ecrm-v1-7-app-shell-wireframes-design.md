...
- “模式”菜单只作为现场模式状态提示入口，不进入独立页面；默认显示“未锁定”，开启现场模式后显示“现场模式”，点击后显示 3 秒 Tips，用于提示未开启或已锁定状态。
- 桌面版补充主要弹窗和子页面交互原型，包括新增商品、编辑商品、复制商品、购物车抽屉、去收款、订单详情、现场模式 PIN、运营赠礼 / 人工赠送 / 其他出库选择。
- 桌面版新增商品表单包含“系列（筛选）”、SPU 编码、SKU 编码、只读完整商品编码预览；售卖页只有在可售商品中存在 2 个及以上不同系列时才显示系列筛选，并放在 SPU 筛选上方。
- 订单页采用两列列表预览，按订单生成顺序两两成组，组内左旧右新，最新一组显示在最上方；例如 8 单时显示为 `007 008 / 005 006 / 003 004 / 001 002`。订单详情需要表达正式版的信息层级，包括基本信息、售后记录、商品明细、库存摘要、完整流水和订单操作。
- 数据页补齐正式版高优先级简易仪表盘分区，包括经营概览、销售表现、成本毛利、活动成本、非销售出库、库存与异常。
- 设置页现场模式补齐两次 PIN 输入确认、锁定范围多选展示、更新 PIN、立即重新锁定、关闭现场模式等入口；基础信息和收款码位于现场模式下方。
